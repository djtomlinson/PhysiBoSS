    ...
    for i in range(-nx, nx + 1):
        for j in range(-ny, ny + 1):
            ...
            if np.sqrt(x**2 + y**2) <= radius:
                points.append((x, y, 0, 0))  # z = 0, case = 0
...
def generate_external_diffusers(distance, xMin, xMax, yMin, yMax, points=[]):
    ...
    for k in range(0, Nx):
        x = k * distance + xMin
        points.append((x, yMin, 0, 1)) #case = 1
        points.append((x, yMax, 0, 1))
        
    for l in range(0, Ny):
        y = l * distance + yMin
        points.append((xMin, y, 0, 1)) #case = 1
        points.append((xMax, y, 0, 1))
...
distance = (6.25/462)*radius*2.0 #particle size
randomness = 0
points = generate_points_in_circle(radius, 1*distance, randomness)
